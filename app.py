...
if suspect_mode == "กรอกผ่านตารางในเว็บ":
    ...
    if uploaded_file:
        ...
        # คลีนช่องว่างที่ซ่อนอยู่ในชื่อคอลัมน์ Excel
        ...
        for idx, row in df.iterrows():
            ...
            if name != "" and name.lower() != "nan":
                final_suspects.append({
                    "index": len(final_suspects) + 1,
                    "name": name,
                    # จัดการลบ .0 กรณีที่ Excel มองเลขบัตรประชาชนเป็นทศนิยม
                    "id_card": str(row.get("เลขบัตรประจำตัวประชาชน", "-")).replace(".0", ""),
                    "address": str(row.get("ที่อยู่", "-")),
                    "charge": str(row.get("ฐานความผิด", "-"))
                })

# สร้างประโยคฐานความผิดรวม เพื่อส่งไปให้ Word (กรณีพิมพ์รวมในที่เดียว)
charge_list = []
...
st.markdown("**การแจ้งให้ญาติทราบ (ข้อ 3)**")
if len(final_suspects) == 0:
    st.warning("⚠️ กรุณากรอกข้อมูล/อัปโหลดไฟล์ผู้ต้องหาใน ส่วนที่ 3 ก่อน เพื่อให้ระบบดึงรายชื่อมากรอกญาติ")
else:
    for s in final_suspects:
        # ช่องให้กรอกญาติแยกตามผู้ต้องหา
        rel_name = st.text_input(f"ชื่อญาติที่ประสงค์แจ้ง ของผู้ต้องหาที่ {s['index']}: {s['name']}", placeholder="ถ้าไม่มีให้ปล่อยว่างไว้", key=f"rel_{s['index']}")
        s["relative_name"] = rel_name if rel_name.strip() else "ไม่ประสงค์แจ้งให้ผู้ใดทราบ"

# ส่วนแจ้งอัยการ ยึดตามวันจับกุม (ไม่มีช่องให้กรอกเวลา)
...
if st.button("💾 สร้างไฟล์บันทึกจับกุม (Word)", type="primary", use_container_width=True):
    try:
        context = {
            ...
            "charges_text": charges_text # เผื่อกรณีใช้ตัวแปรเดียวในการแสดงข้อหาทั้งหมด
        }
