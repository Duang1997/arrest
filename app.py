...
# --- ตั้งค่าหน้าเว็บ ---
st.set_page_config(page_title="ระบบบันทึกจับกุม", layout="centered") # ใช้ centered เพื่อให้ฟอร์มแคบลงและเรียงอ่านง่าย
...
if suspect_mode == "กรอกผ่านตารางในเว็บ":
    ...
    if uploaded_file:
        ...
        for idx, row in df.iterrows():
            if str(row.get("ชื่อ-นามสกุล", "")).strip() != "" and str(row.get("ชื่อ-นามสกุล", "")) != "nan":
                final_suspects.append({
                    "index": idx + 1,
...
if len(final_suspects) == 0:
    st.warning("⚠️ กรุณากรอกข้อมูลผู้ต้องหาใน ส่วนที่ 3 ก่อน เพื่อให้ระบบดึงรายชื่อมากรอกญาติ")
else:
    for s in final_suspects:
        # ช่องให้กรอกญาติแยกตามผู้ต้องหา
        rel_name = st.text_input(f"ชื่อญาติที่ประสงค์แจ้ง ของผู้ต้องหา: {s['name']}", placeholder="ถ้าไม่มีให้ปล่อยว่างไว้", key=f"rel_{s['index']}")
        s["relative_name"] = rel_name if rel_name.strip() else "ไม่ประสงค์แจ้งให้ผู้ใดทราบ"

# ส่วนแจ้งอัยการ ยึดตามวันจับกุม (ไม่มีช่องให้กรอกเวลาแล้ว)
